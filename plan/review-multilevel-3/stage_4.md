# Этап 4. Танки: рендер — согласованность проекции и стоимость кадра ✅ выполнен
...
**Сделано.** `levelView` получил `setCamera(camera)` и `camera`, `alphaFor`
проецирует обе точки (`levelView.js`), `z` по умолчанию равен `level`;
локальный `Tank` кладёт центр камеры в сервис ДО `alphaFor` и передаёт свой
дробный `z`, дым — свой (`Smoke.js`). Тесты: `tests/client/levelView.test.js`
(пять случаев, в том числе совпадение с формулой фильтра и «без камеры —
поведение прежнее») и два случая в `tests/client/parts/Tank.test.js`
(свой танк публикует камеру, чужой — нет).
...
**Выбран минимальный вариант** (камера считается один раз в `_updateStatic`
и идёт параметром), и вот почему `Map` НЕ читает камеру из сервиса:
параллакс слоёв и клин рампы обязаны работать и тогда, когда локального
танка на сцене нет вовсе (наблюдатель, кадры до спавна) — там сервис отдал
бы `null`, и карта осталась бы без проекции. Раз своя камера у `Map` всё
равно нужна, читать для дыры вторую, из сервиса, — значит зависеть от
порядка отрисовки. Обе камеры берутся из одного трансформа сцены в одном
кадре, поэтому дыра и alpha сущностей совпадают.

**Сделано.** `_updateStatic` считает камеру первым делом и передаёт её в
`_updateSeeThrough(camera)` → `_updateLayerSeeThrough` /
`_updateOccluderSeeThrough` → `_applyHole(target, hole, cfg, camera)`.
Ранний выход по параллаксу остался на месте. Вызовов `cameraCenter` в
`Map` за кадр стало два (статика и динамика — это разные парты, каждый
зовёт по одному) вместо трёх-четырёх на статический слой.
...
**Сделано** ровно так: наборы строятся в конструкторе статической ветки и
обнуляются в `destroy`, `_hasLayerTileAt` / `_hasFloorAt` удалены.
...
**Результат.** `npx eslint .` — чисто; `npm test` — 313 тестов, 30 файлов,
зелено; `npm run build` — собрано; `npm run sim:scenarios` — 12 из 12
зелёные, пороги `divergence.thresholds` не тронуты. Документация обновлена
(`docs/en|ru/architecture.md`, `docs/en|ru/configuration.md`), журнал — под
`## [Unreleased]`, `### Fixed`. Приёмка глазами (`npm run dev`, карта
`overpass`) остаётся за пользователем.
