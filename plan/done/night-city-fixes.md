...
| 11 | Свет с верхнего уровня не освещает рампу | 11 | ✅ выполнен |
| 12 | Фары светят сквозь стены и на бока зданий | 12 | ✅ выполнен |
| 13 | Трассер с моста при выстреле вдаль не виден | 13 | ✅ выполнен |
| 14 | Горка ловит фары сбоку и из-за торца | 14 | ✅ выполнен |
...
## Этап 11. Свет с верхнего уровня не освещает рампу (проблема 11) ✅ выполнен
...
- П. 6 — ручная проверка пользователем пройдена.

---

## Этап 12. Фары светят сквозь стены и на бока зданий (проблема 12) ✅ выполнен
...
- П. 1 (по коду, без прогона в браузере): причина подтверждена. Конус — спрайт `textures.cone` без геометрии карты.
  Скриншот 1: стена канала — один ряд `T.CANAL_WALL` (уровень 0, слой 4, объём 0.25); у танка вплотную конус
  начинается у стены, узкое начало закрыто вершиной стены (`setTops`, полумрак поверх источников), а широкая часть —
  на дороге за стеной. Скриншот 2: крыша тёмная из-за той же вершины объёма (`setTops`), боковые грани (перекрыватель
  `levelZ(5, L)` под картой `levelZ(40, L)`) и пол за углом освещаются.
- Препятствия — клетки объёмов уровня (то, что уже отдаёт `setVolumeTops`: стены зданий и канала, перила уровня 1);
  сервис собирает из них сетку `map.blockers` (`Uint8Array` на уровень) в `syncLevels`. Размер сетки — из
  `acquireMap(..., size)`.
- П. 2: `lightMath` — `castRay` (DDA по граням клеток), `firstHit`, `coneFan` (веер `rays` лучей по прямоугольнику
  текстуры с размытыми краями, флаг `clipped`), `fanUvs`, `fanIndices`.
- П. 3 и 4 — выбор способа: **веер-меш вместо маски.** Обрезанный конус рисуется `Mesh` по полигону видимости с той
  же текстурой (UV — `fanUvs`, проекция — трансформ меша `p·(1+k) − cam·k`), в `lights`/`rampLights` карты
  (`LevelLightMap.fanPool`/`rampFanPool`). Стенсил-маска на каждый конус не нужна, меш батчится. Необрезанный конус —
  прежний спрайт. Кеш веера — `WeakMap` по источнику, ключ: позиция, поворот, уровень, радиус, версия сетки. Грани
  объёмов отдельной маски не требуют: веер кончается на следе стены, а видимые грани рисуются поверх следа; вершины
  по-прежнему закрыты `setTops`. See-through-дыра и карта крыш получают те же элементы — без изменений.
- П. 5: отсвет — радиальный источник на оси перед точкой упора (центр отнесён на `0.6·radius` от стены), сила
  `intensity фары · bounce.intensity · (1 − d/length)`, в счёт `maxLights`.
- П. 6: `render.js → lighting.headlights.occlusion { enabled: true, rays: 40 }`, `.bounce { intensity: 0.35, radius: 30,
  maxDistance: 80 }`; при `enabled: false` — прежний спрайт и без отсвета.
- Сверх плана: `lightsAt` пропускает фару, у которой линия видимости до точки перекрыта (нет засвета за стеной).
- П. 7: тесты — `lightMath` (`castRay`, `firstHit`, `coneFan`: пустое поле, стена поперёк, сбоку, вплотную; `fanUvs`,
  `fanIndices`), `createLighting` (веер у стены / спрайт в поле, отсвет только при упоре и в пределах `maxDistance`,
  `maxLights`, выключенная окклюзия, кеш веера, меш в карте, засвет за стеной), `tests/config/lighting.test.js`.
- П. 9: `configuration.md` (`headlights.occlusion`, `.bounce`), `architecture.md` («Фары и стены»), en + ru; CHANGELOG
  `### Fixed`.
- `npx eslint .`, `npm test` (825), `npm run build`, `npx vimp-contract` — зелёные.
- П. 8 — ручная проверка пользователем пройдена («всё хорошо», после правок веера ниже).
- По ручной проверке: у стены/горки конус становился резким — веер на угол клина срезал размытый край текстуры у
  фары (у вершины он шире клина на постоянный отступ). Веер теперь на ±90° от оси до края прямоугольника текстуры,
  `rays: 64`. Поперечная черта у фары — граница веера: размытие текстуры позади вершины срезалось; веер замкнут
  назад несколькими лучами до края текстуры за фарой (`alongBack`, `fanIndices(..., closed)`).

---

## Этап 13. Трассер с моста при выстреле вдаль (проблема 13) ✅ выполнен

Жалоба (ручная проверка этапа 12): танк на эстакаде/мосту стреляет вдаль — трассера не видно. Прежняя правка
(ночью трассер над картой освещённости) не помогла.

### Причина

Промах: ядро берёт уровень конца по сегментам луча (`miss_level` → `shot_levels::ray_segments`), эстакада короче
дальности 1500 — луч «падает» за кромку, `endLevel = 0`. `ShotEffectController` рисовал весь трассер на уровне
конца — под плитой уровня 1 и в проекции земли.

### Ход выполнения

- Ядро: `ClientCore.shot_segments(x, y, dx, dy, range, level)` — плоские `[t0, t1, level, …]` из
  `shot_levels::ray_segments` (тест в `tests/core/clientCore.test.js` на слоёной фикстуре).
- `shots.path(x0, y0, x1, y1, level)` (`shotEvents.js`, ядро — через `createShotEvents({ segments })` в
  `index.js`).
- `tracerPieces.js`: непересекающиеся куски; в окне кромки выигрывает сегмент, начатый раньше (наземный трассер не
  прыгает на плиту); уровень последнего куска — `W1_END_LEVEL` (попадание в цель на кромке — кусок от начала окна).
- `TracerEffect`: `options.pieces` / `layerFor(level)`, подотрезки затухания режутся по кускам, графика на уровень.
- `ShotEffectController`: `layers` (level → контейнер на сцене) вместо `glow`: днём кусок уровня конца — в
  контроллере, остальные — `levelZ(2, L)`; ночью все — `levelZ(45, L)`; проекция, `alphaFor`/`tintFor` по уровню
  слоя (точка — середина его куска).
- Тесты: `tracerPieces.test.js`, `shotEvents.test.js` (`path`), `ShotEffectController.test.js` (слои, разрез на
  кромке, ночь, проекция, уничтожение). Документация: `core.md` (ABI), `architecture.md` (сервис `shots`, эмиссив),
  en + ru. CHANGELOG `### Added` / `### Fixed`.
- Ручная проверка пользователем пройдена.

---

## Этап 14. Горка ловит фары сбоку и из-за торца (проблема 14) ✅ выполнен

Жалоба: танк на земле сбоку от клина рампы или за её верхним торцом (под парковкой) — склон освещён его фарами.

### Причина

Сетка препятствий этапа 12 — только клетки объёмов; клетки рамп для света уровня подножия прозрачны.

### Ход выполнения

- `lightMath.rampBlocks`: вход через подножие — свет идёт по полосе (склон освещён), выход из полосы — стоп; вход
  через борт/верхний торец — стоп, если клин в точке входа выше фары больше `RAMP_CLEARANCE = 0.2`. `castRay`
  отдаёт в `isBlocked` соседнюю клетку и точку входа.
- Сервис: `map.rampCells` (полосы `setRampWedges`, уровень подножия) в `syncBlockers`; `obstaclesFor(light)` —
  стены + рампы; фара на самой рампе от правила освобождена; в ключе кеша веера — `z`.
- Тесты: `lightMath` (`rampBlocks`, `castRay` по склону/сбоку), `createLighting` (борт, торец, подножие, фара на
  рампе, засвет). Документация: `architecture.md` («Фары и стены»), en + ru. CHANGELOG `### Fixed`.
- Ручная проверка пользователем пройдена.
