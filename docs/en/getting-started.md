...
- **Rust toolchain** (`rustup` + `wasm-pack`) — required to build this
  ...
  load;
- **ffmpeg** (`brew install ffmpeg`) — required by `npm run audio:process`,
  the only step that produces the game's sounds. Without it the game has no
  sound at all.
...
npm run audio:process    # sounds → build/sounds/ (ffmpeg) — before build, or there is no sound
...
`dist/img/`) and the sound assets — everything the engine's master
...
> `npm run audio:process` alone (ffmpeg). On a fresh checkout `build:assets`
> therefore stops with `build/sounds/ not found. Run 'npm run audio:process'
> first.` Unlike games generated by `create-vimp-game`, tanks has no
> placeholder sounds to fall back to. Rerun it whenever
> `assets/audio-raw/` changes; CI runs it before every build.
...
- **sounds** — `build/sounds/`, the product of `npm run audio:process`.
  Skip that step (or run it without ffmpeg) and the game has **no sound at
  all** — the match itself still plays.
