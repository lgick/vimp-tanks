...
| `Ramp` | центр корпуса на клетке рампы | **оба** уровня | обычный |
| `Falling` | съехал с обрыва | пустая — не задевает ничего | заблокирован |
...
- **Обрывы.** Танк уровня 1 над тайлом без плиты переходит в `Falling`.
  ...
  игнорируется (удерживаемые клавиши подхватятся при приземлении), тело ни
  с чем не сталкивается и летит по инерции. При приземлении танк на уровне
...
Последнее слово всё равно за кадром: `track_frame` отдаёт `level` строки в
`Predictor::correct_level`, а тот применяет его **только пока реплика в
`Grounded`**. На рампе кадр отстаёт на буфер интерполяции, и коррекция там
отматывала бы подъём назад на каждом тике. Незавершённое `Falling`
снимается перед реплеем реконсиляции: реплей начинается с авторитетной
позиции, к которой падение из отброшенной ветки предсказания не относится.
...
мостом. У падающего танка маска пуста, поэтому контактов он не даёт вовсе,
а подсистемы продолжают шагать.

`ShotPredictor` режет луч тем же `ray_segments()`, что и хост, и каждый
сегмент видит только стены, ящики и корпуса своего уровня; полученные
`startLevel`/`endLevel` уходят в строку локального трассера, а локально
