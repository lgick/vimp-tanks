...
авторитетная сущность, `shot.rs` **не** подменяет одну другой: авторитетная
строка убирается из кадра, а её id запоминается как **алиас** локального
(`bomb_aliases`). Всё, что дальше приходит под авторитетным id — в первую
очередь `null` детонации, — переименовывается в локальный id, так что
сущность живёт под одним именем от спавна до детонации. Удаление и
пересоздание перезапустило бы её таймер и оборвало одноразовый сэмпл
постановки. Алиас снимается по `null` детонации; `BOMB_ALIAS_MAX_AGE`
