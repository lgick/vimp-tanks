...
Каркас инфраструктурный: подсистемы, которые его наполнят (динамика карты,
контактирующие чужие танки), ещё не написаны, но труба до движка уже
проложена. `TanksClient` реализует три хука `GameClientDef` для тел,
...
| --- | --- | --- |
| Rust unit | `core/src/*` (`#[cfg(test)]`) | BodyTag, раскладка кадра; предикт (replay/visualError/freeze, проход столкновений со стенами и предсказанными телами), каркас предсказанного мира (захват, ошибка, возврат в интерполяцию, реконсиляция), выстрелы (гейты/дедуп/RTT) |
