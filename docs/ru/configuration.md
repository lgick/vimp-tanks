...
| `coreParams.levels` | `fallTime: 0.35, fallDamage: 15` | Правила 2.5D-уровней, которые движок довозит до Rust-ядра игры как есть (`coreParams` он не читает и не валидирует): длительность падения с эстакады в секундах и урон при приземлении (`0` — падение бесплатно). Одноуровневая карта их не касается |
...
- **`componentDependencies`** — какие сервисы инжектируются в компоненты
  ...
  ShotEffect, Bomb, Tank; `mapDynamics` → ShotEffect; `levelView` → Tank, Map;
  `localPlayer` → Tank). `mapDynamics` —
...
сервис самой игры (`src/client/levelView.js`): где и на каком уровне
локальный игрок. Пишет его локальный `Tank` (что он локальный, ему сообщает
движковый сервис `localPlayer`), читают слои `Map` уровня >= 1, чтобы гаснуть
над игроком (см. [architecture.md](architecture.md)).
...
У `c1`/`c2` объявлен `optionalFrom: 3`: динамический элемент карты шлёт `[vx, vy, angvel]`,
...
Четыре карты: `pool mini` (малая), `canopy`, `garden` и `overpass` (демо
2.5D). Каждая описывает слои тайлов (`layers`, `tiles`), точки респауна
...
поэтому карта без полей ниже грузится ровно как раньше. Эталон —
`overpass.js`: земля плюс сквозная эстакада с перилами, двумя рампами и
двумя разрывами перил.

| Поле | Значение |
| --- | --- |
| `levels` | Надземные уровни, ключ — номер уровня строкой (`1`). `MAX_LEVELS = 2`, поэтому сегодня только `"1"`; уровни обязаны идти с 1 без пропусков |
