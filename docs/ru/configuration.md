...
### Рендер 2.5D: `seeThrough`, `parallax`, `volume`, `shadow`

Все четыре объекта приходят из `src/config/render.js` и лежат здесь как часть
...
Строка танка (`m1`) заканчивается полями `angvel`, `z` и `level`: `z` —
визуальная высота `0.0..1.0`, она интерполируется (подъём по рампе и
падение обязаны выглядеть плавно), а `level` — дискретный 2.5D-уровень без
промежуточных значений, он переключает zIndex и набор коллизий. Ширина и
