...
- **`bakedAssets`** — процедурные текстуры, «запекаемые» один раз при
  ...
  танк, его тень, бейдж уровня, бомба, следы гусениц, отметки радара. Каждая запись: `name` (id
  ...
  нарисованного круга без запаса под размытие. `tankShadowTexture` — тот же
  baker ещё раз: тёмный широкий круг под танком, самый дешёвый и самый
  читаемый признак высоты (2.5D).

  `levelBadgeTexture` печёт не одну текстуру, а **массив по уровням**
  (кружок с номером уровня в цвете уровня, та же палитра, что у радара, —
  `src/client/levelColors.js`): уровень меняется на лету, а `Text` в каждом
  кадре — это заново растеризованный шрифт.
...
- **`componentDependencies`** — какие сервисы инжектируются в компоненты
  (`renderer` → Map, MapVolume, Tank; `assetsBase` → Map, MapVolume;
  `soundManager` → ExplosionEffect, ShotEffect, Bomb, Tank; `mapDynamics` →
  ShotEffect; `levelView` → Tank, Map, MapVolume, MapRadar, Smoke, Bomb,
  ShotEffect, ExplosionEffect; `localPlayer` → Tank). `mapDynamics` —
...
слои карты и ящики на них, чужие танки, дым, бомбы и эффекты (см.
...
### `parts.seeThrough`, `parts.volume` — рендер 2.5D

Оба объекта приходят из `src/config/render.js` и лежат здесь как часть
...
поэтому `src/client/levelView.js` и `src/client/parts/MapVolume.js`
...
| `volume` | Значение |
| --- | --- |
| `enabled` | `false` полностью выключает `MapVolume` — путь отхода на слабой машине |
| `slices` | Число срезов экструзии на слой: весь эффект стоит `slices` вызовов отрисовки независимо от числа стен |
| `shear` | Насколько верхний срез сдвигается на единицу высоты, долей расстояния до камеры |
| `sideTint` | Затемнение нижних срезов — боковых граней блока |
...
angvel]`: высота и уровень стоят в ГОЛОВЕ, а не в хвосте, потому что
...
| `volumes` / `levels[n].volumes` | Необязательное, **только визуал**: `zIndex рендер-слоя` → его высота в уровнях. Слой с высотой экструдирует парт `MapVolume`, и он смещается при движении камеры. Движок валидирует значение и кладёт его парту в `data.volume` |
