...
describe('Map: слои 2.5D', () => {
  ...
  describe("режим 'hole'", () => {
    it('под плитой появляется фильтр дыры с центром в игроке', () => {
      ...
      const uniforms = bridge._hole.filter.resources.holeUniforms.uniforms;
    ...
    it('над плитой фильтр снимается совсем', () => {
      ...
      expect(bridge._hole.attached).toBe(false);
      expect(bridge._hole.filter).toBe(null);
    });
...
describe('Map: владение ассетами при смене карты', () => {
  ...
  it('destroy построенного слоя освобождает запечённую текстуру и не падает', async () => {
    ...
    expect(map.mapSprite).not.toBe(null);
...
describe('Map: параллакс и объём слоя', () => {
  ...
  const make = (data, levelView) =>
    new Map(
      data,
      {},
      { renderer: viewRenderer, assetsBase: '/build/', levelView },
    );
  ...
  it('слой уровня 1 получает трансформ параллакса, слой уровня 0 — нет', () => {
    ...
    bridge.mapSprite = new Sprite();
    makeStage(bridge);
    bridge.onRender();

    const k = 1 * parallax.shear;

    expect(bridge.mapSprite.scale.x).toBeCloseTo(0.5 * (1 + k), 6);
    expect(bridge.mapSprite.position.x).toBeCloseTo(-CAM_X * k, 6);
    expect(bridge.mapSprite.position.y).toBeCloseTo(-CAM_Y * k, 6);

    // плоский слой земли не платит даже за колбэк
    const ground = make({ ...staticData, level: 0 }, createLevelView(seeThrough));

    expect(ground._onRender).toBe(null);
    expect(ground._parallaxK).toBe(0);
  });
  ...
  describe('экструзия', () => {
    ...
    it('слой с объёмом печётся ОДИН раз и даёт ровно slices срезов', async () => {
      ...
      expect(map._slices).toHaveLength(volume.slices);
      // срезы делят текстуру с плоским слоем
      for (const slice of map._slices) {
        expect(slice.target.texture.source).toBe(map.mapSprite.texture.source);
      }
    });
    ...
    it('срезы объёма живут в перекрывателе — сиблинге парта на сцене', async () => {
      ...
      expect(map._occluder.parent).toBe(stage);
      expect(map._occluder.zIndex).toBeGreaterThan(levelZ(3, map._level));
      // и ниже следующего уровня целиком
      expect(map._occluder.zIndex).toBeLessThan(levelZ(1, map._level + 1));

      for (const slice of map._slices) {
        expect(slice.target.parent).toBe(map._occluder);
      }
    ...
    it('клин рампы остаётся ребёнком парта', async () => {
      ...
      expect(map._occluder).toBe(null);
    ...
    describe('прозрачность перекрывателя', () => {
      ...
      it('стена своего уровня не гаснет, пока не закрывает танк', async () => {
        ...
        expect(map._occluderHole.attached).toBe(false);
        expect(map._occluderHole.strength).toBeLessThan(0.01);
      });
      ...
      it('стена своего уровня гаснет, когда накрывает танк', async () => {
        ...
        expect(map._occluderHole.attached).toBe(true);
        expect(map._occluderHole.strength).toBeGreaterThan(0.9);
      });
    ...
    it('юбка текстурируется изнутри прогона, а не с его кромки', async () => {
      ...
      const uvs = map._slices[0].target.geometry.uvs;
      const width = map._rampTexture.width;
      const height = map._rampTexture.height;
    ...
    it('destroy снимает перекрыватель со сцены и не трогает общую текстуру', async () => {
      ...
      const occluder = map._occluder;
      const source = map._slices[0].target.texture.source;
    ...
    it('срезы стоят по возрастанию высоты и сдвинуты сильнее слоя', async () => {
      ...
      const heights = map._slices.map(slice => slice.k);

      expect(heights).toEqual([...heights].sort((a, b) => a - b));
      expect(heights[0]).toBeGreaterThan(map._parallaxK);

      const top = map._slices[map._slices.length - 1].target;

      expect(Math.abs(top.position.x)).toBeGreaterThan(
        Math.abs(map.mapSprite.position.x),
      );
    });
    ...
    it('рампа даёт свою текстуру, поверхность прогона и юбку', async () => {
      ...
      expect(map._rampTexture).not.toBe(null);
      // прогон — наклонная плоскость плюс юбка насыпи, а не лестница из
      // срезов: два меша, и юбка нарисована первой
      expect(map._slices).toHaveLength(2);
      expect(map._slices[0].base).toBeInstanceOf(Float32Array);
      expect(map._slices[1].base).toBeInstanceOf(Float32Array);
      // юбка стоит на базовой плоскости прогона
      expect(map._slices[0].k).toBeCloseTo(0, 6);
      // порядок отрисовки задаётся вершиной клина: полный перепад рампы
      expect(map._slices[1].k).toBeCloseTo(parallax.shear, 6);
    });
    ...
    it('юбка тянет поверхность прогона вниз, до базовой плоскости', async () => {
      ...
      const skirt = map._slices[0];
      const surface = map._slices[1];
    ...
    it('камера двигает вершины клина тем сильнее, чем они выше', async () => {
      ...
      const { target, base, heights } = map._slices[1];
      const shift = i =>
    ...
    it('рампу чужого рендер-слоя парт не рисует', async () => {
      ...
      expect(bakeTileLayer).toHaveBeenCalledTimes(1);
      expect(map._slices).toHaveLength(0);
    });
    ...
    it('destroy освобождает текстуру клина', async () => {
      ...
      const rampTexture = map._rampTexture;
      const destroy = vi.spyOn(rampTexture, 'destroy');
      const meshes = map._slices.map(slice => slice.target);
