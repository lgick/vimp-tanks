...
pub struct PredictedBody {
    ...
    /// Тело ЗАКОННО поднимается по прогону рампы: только такое тело хост
    /// пропускает сквозь стражей (`map::body_filter`). Заполняет
    /// `client::predictor` одним правилом на все стороны
...
pub trait PredictedBodies {
    ...
    /// Поднимаются ли тела подсистемы по прогонам рампы. Тела карты — нет:
    /// хост им `levels_interaction_on_ramp` не ставит НИКОГДА (ящик,
    /// столкнутый на прогон, упирается в борт), поэтому стражей они видят
    /// всегда. Танки — да, и их вердикт даёт `level::body_on_ramp`.
